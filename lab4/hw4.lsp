...
; Require each pair of variables to have at least one false variable
(defun at-most-one-color-helper (n c var k)
  (cond ((> var k) nil)
	((equal var (- k 1)) (cons (list (* -1 (node2var n c k)) ;=
				     (* -1 (node2var n (+ var 1) k))) ())) ;nil
	(T (cons (list (* -1 (node2var n c k))
			       (* -1 (node2var n (+ var 1) k)))
		   (at-most-one-color-helper n c (+ 1 var) k))))
)

(defun at-most-one-color (n c k)
  (cond ((equal c k) ()) ;nil = try cons list instead of append
	(T (append (at-most-one-color-helper n c c k)
		   (at-most-one-color n (+ 1 c) k)))
)
)
...
(defun generate-edge-clauses-helper (i j m k)
  (cond ((> m k) nil)
        (T (cons (list (* -1 (node2var i m k)) (* -1 (node2var j m k)))
                   (generate-edge-clauses-helper i j (+ 1 m) k))))
  )           ;
